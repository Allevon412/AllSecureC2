...
#define GET_ADDRESS_OF_RETURN_ADDRESS() __builtin_frame_address(0)
#endif
...
int main() {
    ...
    */
    ...
    f = (PRUNTIME_FUNCTION)RTFindFunctionByAddress((UINT64)kernel32Base, (UINT64)pBaseThreadInitThunk - (UINT64)kernel32Base);
    ...
    stackSizeOf = 0;
    f = NULL;
    f = (PRUNTIME_FUNCTION)RTFindFunctionByAddress((UINT64)ntdllBase, (UINT64)pRtlUserThreadStart - (UINT64)ntdllBase);
    ...
    */
    ...
    returnAddress = (PVOID)GET_ADDRESS_OF_RETURN_ADDRESS();
...
VOID SpoofCallStack(PSPOOFER psConfig) {
    ...
    psConfig->ReturnAddress = GET_ADDRESS_OF_RETURN_ADDRESS();
    spoof_call(psConfig);
}
*/
...
PIMAGE_RUNTIME_FUNCTION_ENTRY RTFindFunctionByAddress(UINT64 modulelBase, DWORD64 functionOffset) {
    ...
    for (DWORD i = 0; i < pImageExportDirectory->NumberOfFunctions; i++)
    {
        if (pRuntimeFunctionTable[i].BeginAddress == functionOffset) {
            ...
            return (PIMAGE_RUNTIME_FUNCTION_ENTRY)pRuntimeFunctionTable + i;
        }
    }
    return NULL;
}
...
PIMAGE_RUNTIME_FUNCTION_ENTRY RTFindFunctionByIndex(UINT64 kernelBase, DWORD index) {
    ...
    return (PIMAGE_RUNTIME_FUNCTION_ENTRY)pRuntimeFunctionTable + index;
}

...
*/
	...
	EncryptLength = pack->Length - Offset;

	if(EncryptLength > 0)
	{
		AESCTR((BYTE*)pack->Buffer + Offset, EncryptLength, agent->AESKey, agent->AESKeySize, agent->IV);
	}

	//send the big package.
	if (PerformRequest(pack->Buffer, pack->Length, Response))
	{
		Success = TRUE;
	}
	else {

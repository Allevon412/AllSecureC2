...
func GetListenerData(databasepath string) ([]Types.ListenerData, error) {
	var (
		db          *sql.DB
		err         error
		ListenerRow *sql.Rows
		listdata    Types.ListenerData
	)

	db, err = sql.Open("sqlite3", databasepath)
	if err != nil {
		log.Println("[error] Failed to open database", err)
		return []Types.ListenerData{}, err
	}
	defer db.Close()
...
SELECT u.username, ld.user_id, ld.listener_name, ld.protocol, ld.host, ld.port_bind FROM users u JOIN listeners ld ON u.id = ld.user_id`
	...
	var data_slice []Types.ListenerData
...
func AddListenerToSQLTable(username, databasepath string, userid int, data Types.ListenerData) error {
	var (
		db   *sql.DB
		err  error
		stmt *sql.Stmt
...
func RemoveListenerFromSQLTable(databasepath string, data Types.ListenerData) (Types.ListenerData, error) {
	var (
		db          *sql.DB
		err         error
		stmt        *sql.Stmt
...
func AddImplantToSqlTable(databasepath string, userid int, data Types.ImplantData) error {
	var (
		db   *sql.DB
		err  error
		stmt *sql.Stmt

...
func CheckIfAgentIsAlive(Agent *Implant, index int) bool {
	...
	if Agent.Alive && Agent.LastCheckin != (time.Time{}) && Agent.Context.Sleep_delay != 0 {
		if Agent.LastCheckin.Add(time.Duration(float32(Agent.Context.Sleep_delay)*1.5) * time.Second).Before(time.Now()) {
			Agent.Alive = false
			log.Println("[info] agent [", Agent.Context.Agent_name, "] index [", index, "] has not checked in for ", float32(Agent.Context.Sleep_delay)*1.5, " seconds")
			return false
		}
	}
	return true
